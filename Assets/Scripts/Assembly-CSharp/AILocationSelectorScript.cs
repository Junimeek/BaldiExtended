...
public class AILocationSelectorScript : MonoBehaviour
{
	...
	{
		this.id = Mathf.RoundToInt(UnityEngine.Random.Range(0f, totalCount-1f)); //Get a random number between 0 and 28
	...
	{
		this.id = Mathf.RoundToInt(UnityEngine.Random.Range(0f, hallCount-1f)); //Get a random number between 0 and 15
		base.transform.position = this.newLocation[this.id].position; //Set it's location to a position in a list of positions using the ID variable that just got set.
		this.ambience.PlayAudio(); //Play an ambience audio
	}
	...
	{
		this.id = Mathf.RoundToInt(UnityEngine.Random.Range(1f, 25f)); //Get a random number between 0 and 15
		base.transform.position = this.newLocation[this.id].position; //Set it's location to a position in a list of positions using the ID variable that just got set.
	}
